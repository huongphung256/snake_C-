...
namespace demo_drawing
{
    public partial class frmMain : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            getData.DataConnect();
            getData.DataQuery();
            label12.Text += getData.data.Rows[0][0];
            getData.DataDisconnect();
        }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i < doDai; i++)
                if (x[0] == x[i] && y[0] == y[i])
                {
                    ...
                    //}
                    int pointCurrent = Convert.ToInt32(lbDiem.Text);
                    if (pointCurrent > Convert.ToInt32(getData.data.Rows[0][0]))
                    {
                        getData.DataConnect();
                        getData.DataAdd(pointCurrent);
                        getData.DataDisconnect();
                    }
                    Application.Restart();
                }
